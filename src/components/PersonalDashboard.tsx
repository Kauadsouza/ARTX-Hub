...
export function PersonalDashboard({ tasks, notes, systemSignals, syncError, syncing, onOpen, onCreate, onToggle, onNote, onRetry, onBackup }: {
  ...
  return <div className="overview-page personal-dashboard page-enter">
    ...
    <section className="system-health"><div className="system-health-heading"><div><p className="eyebrow">{t("SAÚDE DOS SISTEMAS")}</p><h2><Activity size={19} />{t(" Estado real das integrações")}</h2></div><button onClick={onBackup}><Download size={15} />{t(" Exportar Hub")}</button></div><div className="health-grid">{([['site','Site KauaArtx'],['videos','KauaArtx Video Studio'],['sat','Idiomas'],['university','University Path']] as const).map(([key,label]) => { const signal = systemSignals[key]; return <button key={key} onClick={() => onOpen(key)} className={signal?.state ?? 'unknown'}><span><ShieldCheck size={16} />{t(label)}<i /></span><strong>{t(signal?.title) ?? t("Abrir para verificar")}</strong><small>{t(signal?.detail) ?? t("O Hub registra aqui o último estado confirmado pelo sistema.")}</small>{signal?.updatedAt && <time dateTime={signal.updatedAt}>{new Intl.DateTimeFormat(locale,{hour:'2-digit',minute:'2-digit',timeZone:'Europe/London'}).format(new Date(signal.updatedAt))}</time>}</button>; })}</div></section>
